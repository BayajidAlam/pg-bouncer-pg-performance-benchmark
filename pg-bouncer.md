...
Create a new database and user, and assign privileges:
...
Open the pgbouncer.ini file to update the configuration.
...
In Grafana's left sidebar, click on the gear icon (Configuration).
Click on Data Sources.
Click Add data source.
Choose Prometheus from the list of available data sources.
In the HTTP section, set the URL to http://<MONITORING_SERVER_PRIVATE_IP>:9090
...
In Grafana's left sidebar, click on the plus icon (Create) and select Import.
For PostgreSQL metrics, import the dashboard with ID 9628.
For PgBouncer metrics, import the dashboard with ID 13125.
Once the dashboards are imported, you should be able to visualize various metrics related to PostgreSQL and PgBouncer.
PostgreSQL metrics
...
Connect to PostgreSQL as a Superuser: Connect to PostgreSQL using a superuser account (e.g., postgres):

sudo -u postgres psql
Grant Permissions: Grant the necessary permissions to myuser for the mydb database:
...
Active Connections: Check the number of active client and server connections:

pgbouncer_pools_client_active_connections
pgbouncer_pools_server_active_connections
alt text

Waiting Connections: Check the number of clients waiting for a server connection:

pgbouncer_pools_client_waiting_connections
Query Throughput: Check the rate of queries being executed:

rate(pgbouncer_stats_queries_pooled_total[1m])
alt text

Transaction Throughput: Check the rate of transactions being executed:
...
Create a Script: Create a file called test.sql with your custom queries:

BEGIN;
SELECT * FROM pgbench_accounts WHERE aid = 1;
UPDATE pgbench_accounts SET abalance = abalance + 1 WHERE aid = 1;
COMMIT;
Run the Benchmark: Run pgbench with the custom script:

pgbench -h localhost -p 6432 -U myuser -c 10 -j 2 -t 100 -f test.sql mydb
Conclusion
PgBouncer is a powerful and lightweight connection pooler for PostgreSQL that optimizes connection management, reduces resource usage, and improves performance. By using proper configurations and pooling modes, you can ensure your application scales efficiently, even under high-concurrency loads.