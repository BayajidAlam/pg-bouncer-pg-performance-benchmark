...
# Read SSH public key (must be generated by setup-all.sh first)
def get_ssh_public_key():
    """Read existing SSH public key"""
    ssh_dir = os.path.expanduser("~/.ssh")
    public_key_path = os.path.join(ssh_dir, f"{key_name}.id_rsa.pub")
    
    if not os.path.exists(public_key_path):
        raise FileNotFoundError(
            f"SSH key not found at {public_key_path}. "
            f"Please run setup-all.sh or generate the key manually."
        )
...
public_key_content = get_ssh_public_key()
